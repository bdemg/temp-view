...
def send_overheat_alert(sensor, temperature_reading):
    ...
    mail_content = overheat_message.format(
        ...
        temperature_reading,
        sensor.upper_temperature_limit
    )
    send_email_async(subject=mail_subject, content=mail_content, to=AlertEmails.objects.values_list('email', flat=True))
...
def send_freezing_alert(sensor, temperature_reading):
    ...
    mail_content = underheat_message.format(
        ...
        sensor.building.name,
        temperature_reading,
        sensor.lower_temperature_limit
    )
    send_email_async(subject=mail_subject, content=mail_content, to=AlertEmails.objects.values_list('email', flat=True))


@shared_task
def send_email_async(subject, content, to, content_type="text/html"):
    """
    ...
    """
    try:
        msg = EmailMultiAlternatives(subject=subject, body=content, from_email="EMAIL MAN",
                                     to=to)
